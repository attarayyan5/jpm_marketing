  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold font-display" style={{ color: 'var(--text-heading)' }}>Manage Pricing</h1>
            ...
            <tbody>
              {pricing.map(p => (
                <tr key={p.id}>
                  ...
                  <td className="font-medium" style={{ color: 'var(--text-primary)' }}>{p.service_name}</td>
                  <td className="text-gold-500 font-semibold">{p.cost_estimate}</td>
                  <td className="max-w-[250px] truncate" style={{ color: 'var(--text-secondary)' }}>{p.details}</td>
                  <td>
              ...
              {!loading && pricing.length === 0 && <tr><td colSpan={5} className="text-center py-8" style={{ color: 'var(--text-muted)' }}>No pricing entries</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setModal(false)}>
          <div className="glass-card w-full max-w-lg p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold font-display" style={{ color: 'var(--text-heading)' }}>{editing ? 'Edit' : 'Add'} Pricing</h2>
