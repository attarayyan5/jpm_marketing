  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold font-display" style={{ color: 'var(--text-heading)' }}>Manage Services</h1>
            ...
            <tbody>
              {services.map(s => (
                <tr key={s.id}>
                  ...
                  <td className="font-medium" style={{ color: 'var(--text-primary)' }}>{s.title}</td>
                  <td className="max-w-[250px] truncate" style={{ color: 'var(--text-secondary)' }}>{s.description}</td>
                  <td>
              ...
              {!loading && services.length === 0 && <tr><td colSpan={5} className="text-center py-8" style={{ color: 'var(--text-muted)' }}>No services</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setModal(false)}>
          <div className="glass-card w-full max-w-lg p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-bold font-display" style={{ color: 'var(--text-heading)' }}>{editing ? 'Edit' : 'Add'} Service</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center gap-3"><input type="checkbox" id="active-check" checked={form.is_active} onChange={e => setForm({...form, is_active: e.target.checked})} className="w-4 h-4 accent-gold-500" /><label htmlFor="active-check" className="text-sm" style={{ color: 'var(--text-secondary)' }}>Active</label></div>
